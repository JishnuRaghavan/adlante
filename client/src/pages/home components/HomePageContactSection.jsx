import React from 'react'

export default function
  () {
  return (
    <div className='w-screen flex flex-col justify-center items-center bg-slate-300 h-[60vh]'>
      <div className=' bg-sky-100 shadow-xl flex flex-col justify-center items-center rounded-lg p-4'>
        <div className='py-5 flex'>
          <div className="group border-solid border-slate-300 border-2 rounded-full p-1 mx-2 hover:border-gray-500 hover:cursor-pointer">
            <svg
              version={1.0}
              xmlns="http://www.w3.org/2000/svg"
              width="50.000000pt"
              height="50.000000pt"
              viewBox="0 0 50.000000 50.000000"
              preserveAspectRatio="xMidYMid meet"
            >
              <g
                transform="translate(0.000000,50.000000) scale(0.100000,-0.100000)"
                className="group-hover:fill-gray-500"
                fill="rgb(203,213,225)" /* Default fill color */
                stroke="none"
              >
                <path
                  d="M162 450 c-104 -16 -137 -42 -126 -99 5 -25 11 -31 33 -31 48 0 68
11 84 46 l17 34 80 0 80 0 17 -35 c13 -28 24 -36 57 -43 50 -11 58 -5 59 43 1
42 -16 60 -73 75 -55 14 -170 19 -228 10z"
                />
                <path
                  d="M177 343 c-4 -3 -7 -13 -7 -21 0 -8 -22 -38 -49 -67 -55 -58 -71 -99
-62 -165 5 -38 10 -46 30 -48 13 -2 28 1 34 7 15 15 239 15 254 0 6 -6 21 -9
34 -7 20 2 25 10 30 48 9 66 -7 107 -62 165 -27 29 -49 60 -49 69 0 24 -30 33
-45 13 -15 -21 -61 -22 -69 -2 -6 15 -28 20 -39 8z m33 -103 c0 -5 -9 -10 -20
-10 -11 0 -20 5 -20 10 0 6 9 10 20 10 11 0 20 -4 20 -10z m60 0 c0 -5 -9 -10
-20 -10 -11 0 -20 5 -20 10 0 6 9 10 20 10 11 0 20 -4 20 -10z m60 0 c0 -5 -9
-10 -20 -10 -11 0 -20 5 -20 10 0 6 9 10 20 10 11 0 20 -4 20 -10z m-120 -40
c0 -5 -9 -10 -20 -10 -11 0 -20 5 -20 10 0 6 9 10 20 10 11 0 20 -4 20 -10z
m60 0 c0 -5 -9 -10 -20 -10 -11 0 -20 5 -20 10 0 6 9 10 20 10 11 0 20 -4 20
-10z m60 0 c0 -5 -9 -10 -20 -10 -11 0 -20 5 -20 10 0 6 9 10 20 10 11 0 20
-4 20 -10z m-120 -40 c0 -5 -9 -10 -20 -10 -11 0 -20 5 -20 10 0 6 9 10 20 10
11 0 20 -4 20 -10z m60 0 c0 -5 -9 -10 -20 -10 -11 0 -20 5 -20 10 0 6 9 10
20 10 11 0 20 -4 20 -10z m60 0 c0 -5 -9 -10 -20 -10 -11 0 -20 5 -20 10 0 6
9 10 20 10 11 0 20 -4 20 -10z"
                />
              </g>
            </svg>
          </div>

          <div className='border-solid border-slate-300 border-2 rounded-full p-1 mx-2 cursor-pointer hover:cursor-pointer hover:border-gray-500 group'>
            <svg
              version={1.0}
              xmlns="http://www.w3.org/2000/svg"
              width="50.000000pt"
              height="50.000000pt"
              viewBox="0 0 50.000000 50.000000"
              preserveAspectRatio="xMidYMid meet"
            >
              <g className='group-hover:fill-gray-500'
                transform="translate(0.000000,50.000000) scale(0.100000,-0.100000)"
                fill="rgb(203,213,225)"
                stroke="none"
              >
                <path
                  d="M205 471 c-134 -34 -215 -180 -166 -299 10 -25 11 -42 2 -76 -19 -76
-19 -76 45 -59 47 12 62 12 88 2 43 -18 125 -15 166 6 175 88 178 321 5 408
-39 19 -102 27 -140 18z m-4 -132 c6 -12 8 -36 5 -54 -6 -28 -2 -36 25 -59 35
-30 54 -33 67 -10 8 15 12 15 40 0 54 -28 37 -76 -27 -76 -101 1 -224 153
-169 208 19 19 46 14 59 -9z"
                />
              </g>
            </svg>
          </div>
          <div className='border-solid border-slate-300 border-2 rounded-full p-1 mx-2 cursor-pointer group hover:cursor-pointer hover:border-gray-500'>
            <svg
              version={1.0}
              xmlns="http://www.w3.org/2000/svg"
              width="50.000000pt"
              height="50.000000pt"
              viewBox="0 0 50.000000 50.000000"
              preserveAspectRatio="xMidYMid meet"
            >
              <g className='group-hover:fill-gray-500'
                transform="translate(0.000000,50.000000) scale(0.100000,-0.100000)"
                fill="rgb(203,213,225)"
                stroke="none"
              >
                <path
                  d="M206 480 c-41 -13 -94 -68 -106 -108 -12 -47 3 -123 40 -197 36 -70
96 -155 110 -155 14 0 75 86 110 156 37 76 52 150 40 196 -12 43 -66 95 -111
108 -39 11 -44 11 -83 0z m88 -116 c20 -20 20 -68 0 -88 -20 -20 -68 -20 -88
0 -20 20 -20 68 0 88 8 9 28 16 44 16 16 0 36 -7 44 -16z"
                />
              </g>
            </svg>
          </div>
          <div className='border-solid border-slate-300 border-2 rounded-full p-1 mx-2 hover:cursor-pointer hover:border-gray-500 group'>
            <svg
              version={1.0}
              xmlns="http://www.w3.org/2000/svg"
              width="50.000000pt"
              height="50.000000pt"
              viewBox="0 0 32.000000 32.000000"
              preserveAspectRatio="xMidYMid meet"
            >
              <g className='group-hover:fill-gray-500'
                transform="translate(0.000000,32.000000) scale(0.100000,-0.100000)"
                fill="rgb(203,213,225)"
                stroke="none"
              >
                <path
                  d="M30 160 l0 -90 130 0 130 0 0 90 0 90 -130 0 -130 0 0 -90z m228 61
c8 -5 12 -23 10 -49 l-3 -41 -34 21 -34 22 29 23 c39 31 23 29 -26 -3 l-40
-27 -41 27 c-22 15 -43 26 -45 23 -3 -2 7 -12 22 -22 l27 -17 -29 -24 c-16
-13 -32 -23 -36 -24 -14 0 -9 88 5 93 21 9 181 7 195 -2z m-24 -86 c20 -15 36
-31 36 -36 0 -5 -49 -9 -110 -9 -60 0 -110 4 -110 9 0 5 17 22 39 38 49 36 96
36 145 -2z"
                />
              </g>
            </svg>
          </div>
        </div>
        <div className='py-5 text-xl'>
          Hey Handsome...
        </div>
        <div className='py-5'>
          Do you have any questions regarding your order or need help? Please check out our FAQ or contact our customer service. We are happy to help!
        </div>
        <div className='py-5'>
          <button className='hover:bg-black hover:text-blue-100 hover:border-blue-200 border-black border-2 p-2 rounded-lg'>customer service</button>
        </div>
      </div>
    </div>
  )
}
